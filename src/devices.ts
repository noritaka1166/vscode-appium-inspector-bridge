...
const runDeviceCommand: DeviceRunner = (command, args) => new Promise((resolve, reject) => {
  const run = (file: string, fallback?: string): void => {
    execFile(file, args, { timeout: 15_000, maxBuffer: 4 * 1024 * 1024, encoding: 'utf8' }, (error, stdout, stderr) => {
      ...
      reject(new Error(error.killed ? t(`${command}: 確認が15秒でタイムアウトしました。`, `${command}: check timed out after 15 seconds.`)
...
export function parseAndroid(output: string): DeviceReport {
  ...
  if (!output.includes('List of devices attached')) throw new Error(t('adb の端末一覧形式を読み取れませんでした。', 'Could not parse the adb device list.'));
  for (const line of output.split(/\r?\n/)) {
    const match = /^(\S+)\s+(device|offline|unauthorized|no permissions)(?:\s|$)(.*)/.exec(line.trim());
    if (!match) continue;
    const [, udid, state, properties] = match;
    if (state !== 'device') {
      let guidance = t('USBアクセス権を確認してください。', 'Check USB access permissions.');
      if (state === 'unauthorized') guidance = t('端末のUSBデバッグ許可を確認してください。', 'Allow USB debugging on the device.');
      else if (state === 'offline') guidance = t('端末の接続・起動状態を確認してください。', 'Check the device connection and boot state.');
      notes.push(`${udid}: ${state}。${guidance}`);
      continue;
    }
    const model = /(?:^|\s)model:(\S+)/.exec(properties)?.[1].replaceAll('_', ' ');
    devices.push({ id: `Android:${udid}`, platform: 'Android', udid, name: model || udid, state: udid.startsWith('emulator-') ? t('エミュレーター・接続中', 'Emulator · connected') : t('端末・接続中', 'Device · connected') });
  }
  return { devices, notes };
}

const simulatorFormatError = (): Error => new Error(t('simctl の端末一覧形式を読み取れませんでした。', 'Could not parse the simctl device list.'));
...
function parseSimulatorEntry(entry: unknown, runtime: string): Device | undefined {
  ...
  if (!('udid' in entry) || !('name' in entry) || !('state' in entry)
    ...
    throw new Error(t('シミュレーターの情報が不正です。', 'Simulator information is invalid.'));
  }
...
export async function listDevices(run: DeviceRunner = runDeviceCommand, platform = nodePlatform): Promise<DeviceReport> {
  const android = async (): Promise<DeviceReport> => {
    ...
    catch (error) { return { devices: [], notes: [t(`Android一覧を取得できません。Android SDK Platform-Toolsを導入し、PATHまたはANDROID_HOMEを確認してください。\n${String(error)}`, `Could not list Android devices. Install Android SDK Platform-Tools and check PATH or ANDROID_HOME.\n${String(error)}`)] }; }
  };
  const ios = async (): Promise<DeviceReport> => {
    if (platform !== 'darwin') return { devices: [], notes: [t('iOSシミュレーターの一覧はmacOSのみ対応しています。', 'iOS simulator listing is supported on macOS only.')] };
    try { return parseSimulators(await run('xcrun', ['simctl', 'list', 'devices', 'available', '--json'])); }
    catch (error) { return { devices: [], notes: [t(`iOS一覧を取得できません。Xcode・Simulatorランタイムと、xcode-select -p の選択先を確認してください。\n${String(error)}`, `Could not list iOS simulators. Check Xcode, Simulator runtimes, and xcode-select -p.\n${String(error)}`)] }; }
  };
