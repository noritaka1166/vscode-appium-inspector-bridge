...
export async function startInspectorProxy(upstream: URL, adapter: string, bootstrap?: (token: string) => string): Promise<{ url: URL; token: string; close(): void }> {
  ...
  const server = createServer((req, res) => {
    ...
    if (target.origin !== `http://${host}`) { res.writeHead(403).end(); return; }
    const headers = { ...req.headers, host: upstream.host };
    delete headers.origin;
    delete headers['accept-encoding'];
    const forward = request(new URL(target.pathname + target.search, upstream.origin), { method: req.method, headers }, incoming => {
      const responseHeaders = { ...incoming.headers };
      delete responseHeaders['access-control-allow-origin'];
      const html = target.pathname === '/inspector' && incoming.statusCode === 200;
      if (html) {
  ...
  const address = server.address();
  if (!address || typeof address === 'string') throw new Error('Inspector relay failed');
  return { url: new URL(`http://127.0.0.1:${address.port}/inspector`), token, close() { server.closeAllConnections(); server.close(); } };
}
