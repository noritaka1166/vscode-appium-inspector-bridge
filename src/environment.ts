...
function commandNotFound(): Error & { code: string } {
  const error = new Error('appium command was not found.') as Error & {
    code: string;
  };
  error.code = 'ENOENT';
  return error;
        ...
        if (error) {
          ...
          if (code === 'ENOENT')
            ...
            detail = t(
              ...
              `Command failed: ${String(stderr || error.message).slice(0, 1500)}`,
            );
...
function installed(
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error(
      t(
        '一覧のJSON形式を確認できませんでした。',
        'Could not parse the list JSON.',
      ),
    );
...
export async function checkEnvironment(
  ...
  try {
    ...
    if (!match)
      throw new Error(
        t(
          'Appium のバージョンを読み取れませんでした。',
          'Could not read the Appium version.',
        ),
      );
    if (Number(match[1]) < 3) {
      items.push({
        ...
        status: 'error',
        detail: t(
          `${version}（この拡張は Appium 3 以降が必要です）`,
          `${version} (this extension requires Appium 3 or later)`,
        ),
        action: t(
    ...
    items = [
      ...items,
      {
        name: t('Inspector プラグイン', 'Inspector plugin'),
        status: 'skipped',
        detail: t(
          'Appium CLI を確認できないため未確認です。',
          'Not checked because the Appium CLI could not be verified.',
        ),
      },
      {
        name: t('ドライバー', 'Drivers'),
        status: 'skipped',
        detail: t(
          'Appium CLI を確認できないため未確認です。',
          'Not checked because the Appium CLI could not be verified.',
  ...
  const results = await Promise.all(
    (['plugin', 'driver'] as const).map(async (kind): Promise<CheckItem> => {
      const name =
        kind === 'plugin'
          ? t('Inspector プラグイン', 'Inspector plugin')
          : t('ドライバー', 'Drivers');
      try {
        const entries = installed(
          await run([kind, 'list', '--installed', '--json']),
        );
        if (kind === 'plugin') {
          return entries.inspector?.installed
            ? {
                name,
                status: 'ok',
                detail: `inspector ${entries.inspector.version}`,
              }
            : {
                name,
                status: 'error',
                detail: t('未導入です。', 'Not installed.'),
                action: t(
        ...
        return drivers.length
          ...
          : {
              ...
              status: 'warning',
              detail: t(
                'ドライバーがありません。Inspector は開けますが、セッション開始には対象ドライバーが必要です。',
                'No drivers are installed. Inspector can open, but starting a session requires a driver.',
              ),
              action: t(
        ...
        return {
          ...
          status: 'error',
          detail: t(
            `導入状況を確認できませんでした（未導入とは限りません）。\n${error instanceof Error ? error.message : String(error)}`,
            `Could not check installation status (it may still be installed).\n${error instanceof Error ? error.message : String(error)}`,
          ),
          action: t(
            `ターミナルで appium ${kind} list --installed --json を実行し、PATH・APPIUM_HOME とそのアクセス権を確認してください。`,
            `Run appium ${kind} list --installed --json in a terminal, then check PATH, APPIUM_HOME, and access permissions.`,
          ),
        };
      }
    }),
