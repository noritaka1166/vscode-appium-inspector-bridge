(() => {
  ...
  const parentOrigin = globalThis.document?.referrer
    ? new URL(globalThis.document.referrer).origin
    : '*';
  ...
  function endpoint(key, text, from, to) {
    ...
    const servers =
      key === 'SAVED_SESSIONS' ? value.map((item) => item.server) : [value];
    for (const server of servers) {
      const remote = server?.remote;
      if (
        remote &&
        ['127.0.0.1', 'localhost', '::1', '[::1]', undefined, ''].includes(
          remote.hostname,
        ) &&
        String(remote.port) === from
      )
        remote.port = to;
    }
  ...
  try {
    for (const key of keys) {
      if (Object.hasOwn(config.values, key))
        set.call(
          storage,
          key,
          endpoint(key, config.values[key], config.upstreamPort, location.port),
        );
    ...
    function save() {
      ...
      parent.postMessage(
        { bridge: config.token, type: 'saveSettings', values },
        parentOrigin,
      );
    }
    proto.setItem = function (key, value) {
      set.call(this, key, value);
      if (this === storage && keys.includes(String(key))) save();
    };
    ...
    parent.postMessage(
      { bridge: config.token, type: 'error', text: restoreError },
      parentOrigin,
    );
  }
})();
