...
test('clipboard adapter replaces selection and emits input for React; rejects foreign messages', () => {
  const messages = [], listeners = {}, docListeners = {}, dispatched = [];
  class Field {
  ...
  const payload = { bridge: 'test-token', type: 'pasteText', text: '{"platformName":"Android"}' };
...
test('upstream writeText resolves only after host acknowledgement and propagates failure', async () => {
  ...
  let completed = false;
  const first = navigator.clipboard.writeText('android:id/content').then(() => { completed = true; });
  ...
  listeners.message({ source: parent, data: { ...ack, id: messages[1].id, error: 'コピーできません' } });
  await rejected;
});
