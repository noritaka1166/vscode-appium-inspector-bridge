...
test('extension persists hidden-panel settings in secrets and reloads after activation', async () => {
  ...
  function host() {
    ...
    const messages = [];
    const webview = () => ({ asWebviewUri: x => x, cspSource: 'test:', postMessage() {}, onDidReceiveMessage(cb) { this.receive = cb; } });
    const vscode = {
      window: {
        createOutputChannel: () => ({ appendLine() {} }),
        registerWebviewViewProvider: (_, p) => { provider = p; },
        createWebviewPanel: (_, title) => (panel = { title, active: false, webview: webview(), onDidDispose() {} }),
        showErrorMessage: m => messages.push(m)
      },
      commands: { registerCommand() {} }, ViewColumn: { One: 1 },
    ...
    vm.runInNewContext(fs.readFileSync('out/extension.js', 'utf8'), {
      ...
      require: name => {
        ...
        if (name === 'node:fs/promises') return { readFile: async path => path.includes('storage-frame') ? '__INSPECTOR_STORAGE__' : '' };
        if (name === './inspector-proxy') return { startInspectorProxy: async (_, adapter, callback) => {
          bootstrap = callback;
          return { url: new URL('http://127.0.0.1:51000/inspector'), token: 'secret-token', close() {} };
        } };
        return name.startsWith('./') ? require('../out/' + name.slice(2)) : require(name);
      }
    });
    exports.activate({ extensionUri: 'extension', subscriptions: [], secrets: {
      get: async key => data.get(key),
      store: async (key, value) => { writeCount++; await Promise.resolve(); data.set(key, value); }
    } });
    const view = webview();
    ...
    return {
      ...
      stop: exports.deactivate, messages
    };
  }
  const first = host();
  await first.open('http://127.0.0.1:4723');
  const payload = { type: 'saveSettings', bridge: 'secret-token', values: { PREFERRED_THEME: '"dark"', SAVED_SESSIONS: '[{"name":"</script>"}]' } };
