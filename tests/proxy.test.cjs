...
test('relay injects only Inspector HTML, forwards session bodies, rejects other origins', async () => {
  ...
  const upstream = http.createServer(async (req, res) => {
    ...
    res.end(req.url === '/inspector' ? '<html><body>Appium Inspector</body></html>' : JSON.stringify({ value: 'ok' }));
  });
  await new Promise((resolve, reject) => { upstream.once('error', reject); upstream.listen(0, '127.0.0.1', resolve); });
  let relay;
  try {
    relay = await startInspectorProxy(new URL(`http://127.0.0.1:${upstream.address().port}/inspector`), 'const token = __BRIDGE_TOKEN__;');
