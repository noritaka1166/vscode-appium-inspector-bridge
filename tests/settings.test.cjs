...
test('settings restore before app startup across relay ports; saved capabilities untouched', () => {
  ...
  first.localStorage.setItem('SAVED_SESSIONS', JSON.stringify([{ name: 'Android', caps, server: { remote: { hostname: '127.0.0.1', port: '50001' }, cloud: { port: '50001' } } }]));
  ...
  assert.equal(Object.hasOwn(second.messages.at(-1).values, 'PREFERRED_THEME'), false);
...
test('only approved localStorage keys persist; unrelated storage is ignored', () => {
  ...
  assert.deepEqual(validateSettings({ PREFERRED_THEME: '"dark"' }), { PREFERRED_THEME: '"dark"' });
});
