...
const {
  ...
  launcherHtml,
  webviewText,
} = require('../out/official');

test('plugin endpoint ignores Appium base path', () => {
  assert.equal(
    inspectorUrl('http://127.0.0.1:4723/wd/hub').href,
    'http://127.0.0.1:4723/inspector',
  );
  assert.equal(
    inspectorUrl('http://[::1]:4723/').href,
    'http://[::1]:4723/inspector',
  );
});
test('embedded UI rejects credentials, remote hosts and unsafe URL schemes', () => {
  for (const url of [
    ...
    'file:///tmp/a',
    'http://example.org',
    'http://user:pass@localhost:4723',
    'http://localhost:4723/?foo=bar',
    'http://localhost:4723/#fragment',
  ]) {
