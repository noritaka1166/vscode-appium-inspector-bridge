...
test('iOS includes available iOS runtimes, including shutdown devices, but not tvOS', () => {
  const report = parseSimulators(JSON.stringify({ devices: {
...
test('partial enumeration failures preserve working platforms and provide guidance', async () => {
  ...
  assert.deepEqual(calls[1], ['xcrun', ['simctl', 'list', 'devices', 'available', '--json']]);
  const linux = await listDevices(async cmd => { assert.equal(cmd, 'adb'); throw Error('missing'); }, 'linux');
...
test('capability templates select driver and safely encode device names and ids', () => {
  for (const platform of ['Android', 'iOS']) {
    const result = JSON.parse(capabilitiesFor({ platform, udid: 'id"\\', name: '<name>"', state: '', id: 'id' }));
    assert.equal(result.platformName, platform);
    assert.equal(result['appium:automationName'], platform === 'Android' ? 'UiAutomator2' : 'XCUITest');
