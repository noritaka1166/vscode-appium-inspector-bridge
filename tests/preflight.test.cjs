...
function setup({ trusted = true, reachable = false, canStart = false } = {}) {
  ...
  const report = {
    canStart,
    items: [
      { name: 'Appium', status: canStart ? 'ok' : 'error', detail: 'test' },
    ],
  };
  const vscode = {
    env: {
  ...
  vm.runInNewContext(fs.readFileSync('out/extension.js', 'utf8'), {
    ...
    fetch: async (url) => {
      ...
      return {
        ok: String(url).endsWith('/status') && reachable,
        text: async () => '',
      };
    },
    require: (name) => {
      ...
      if (name === 'node:child_process')
        return {
          spawn: () => {
            spawns++;
            throw Error('test: stop before real spawn');
          },
        };
...
test('untrusted workspace never executes diagnostic or server commands', async () => {
  ...
  assert.equal(host.spawns(), 0);
  assert.ok(
    host.events.some((m) => m.type === 'notice' && m.text.includes('信頼')),
  );
});
test('device selection generates and copies only enumerated device capabilities', async () => {
  ...
  const template = host.events.find(
    (m) => m.type === 'capabilitiesTemplate',
  ).text;
