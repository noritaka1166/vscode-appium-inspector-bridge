...
function host() {
  ...
  const vscode = {
    ...
    window: {
      ...
      createWebviewPanel: () => (panel = { active: true, webview: view(), onDidDispose(cb) { dispose = cb; } }),
  ...
  vm.runInNewContext(fs.readFileSync('out/extension.js', 'utf8'), {
    ...
    fetch: async url => ({ ok: String(url).endsWith('/status') ? spawned : true, text: async () => 'Appium Inspector' }),
    require: name => {
  ...
  return { events, dialogs, kills: () => kills, choose: cb => { choose = cb; },
